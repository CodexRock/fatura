...
export interface RecentInvoice extends Invoice {
  clientName: string;
}
...
export function useDashboard() {
  ...
  // Build a fast O(1) lookup map from the shared clients list
  ...
  const dashboardData = useMemo(() => {
    ...
    for (let i = 5; i >= 0; i--) {
      ...
      chartMap.set(key, { label, amount: 0 });
    }
    ...
    invoices.forEach(inv => {
      ...
      if (
        ...
        (balance > 0 && inv.dueDate && inv.dueDate.toMillis() < now.getTime() && (inv.status as string) !== 'overdue')
      ) {
        overdueCount += 1;
      }
    });
    ...
    // Resolve client names for recent invoices
    const recentInvoices: RecentInvoice[] = invoices.slice(0, 5).map(inv => ({
      ...inv,
      clientName: clientsMap.get(inv.clientId) || 'Client inconnu',
    }));
